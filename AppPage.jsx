...
export default function AppPage(){
  ...
  return(
    <div style={{display:'flex',height:'100vh',fontFamily:'system-ui,-apple-system,sans-serif',background:'#f5f4f9'}}>
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        <div style={{background:'#fff',borderBottom:'0.5px solid #eee',padding:'0 22px',height:50,display:'flex',alignItems:'center',gap:8,flexWrap:'nowrap',overflow:'hidden'}}>
          ...
          {!noFilters.includes(view)&&(<>
            ...
            {meetingTypes.length>0&&<select value={filterMeeting} onChange={e=>setFilterMeeting(e.target.value)} style={{padding:'5px 8px',fontSize:12,border:'0.5px solid #ddd',borderRadius:7,background:'#fff',color:'#111',cursor:'pointer',flex:'0 0 auto'}}><option value="all">Tipo reunión</option>{meetingTypes.map(m=><option key={m} value={m}>{m}</option>)}</select>}
          </>)}
          {!noFilters.includes(view)&&<button onClick={()=>setShowForm(true)} style={{background:'#3C3489',color:'#fff',border:'none',borderRadius:7,padding:'6px 12px',fontSize:13,cursor:'pointer',fontFamily:'inherit',fontWeight:500,flex:'0 0 auto',marginLeft:'auto'}}>+ Nueva tarea</button>}
        </div>
        <div style={{flex:1,overflowY:'auto',padding:'18px 22px'}}>
          {loading?<Spinner/>:(<>
            {view==='list'&&<ListView tasks={filteredByWeek} profiles={profiles} onOpen={setOpenTaskId} filterStatus={filterStatus} filterLeader={filterLeader} filterEngineer={filterEngineer} filterMeeting={filterMeeting} setFilterStatus={setFilterStatus} setFilterLeader={setFilterLeader} setFilterEngineer={setFilterEngineer} setFilterMeeting={setFilterMeeting}/>}
            {view==='kanban'&&<KanbanView tasks={filteredByWeek} profiles={profiles} onOpen={setOpenTaskId} filterLeader={filterLeader} filterEngineer={filterEngineer} filterMeeting={filterMeeting}/>}
            {view==='matrix'&&<MatrixView tasks={filteredByWeek} onOpen={setOpenTaskId} filterLeader={filterLeader} filterEngineer={filterEngineer} filterMeeting={filterMeeting}/>}
      ...
      {showForm&&<TaskModal profiles={profiles} me={me} onSave={loadAll} onClose={()=>setShowForm(false)}/>}
    </div>
  )
}