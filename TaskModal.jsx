...
export default function TaskModal({ task, profiles, onSave, onClose }) {
  ...
  const [f, setF] = useState({
    ...
    tag:            task?.tag            || 'Procesos',
    country:        task?.country        || 'Chile',
    meeting_type:   task?.meeting_type   || '',
    meeting_origin: task?.meeting_origin || '',
  })
  ...
  async function save() {
    ...
    try {
      ...
      if (isNew) {
        ...
        if (e) throw e
        await supabase.from('history').insert({ task_id: task.id, actor_id: me?.id, message: 'Tarea actualizada' })
      }
      onSave?.(); onClose()
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.42)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background:'#fff', borderRadius:12, border:'0.5px solid #ddd', width:'100%', maxWidth:580, maxHeight:'90vh', overflow:'auto' }}>
        ...
        <div style={{ padding:'20px 24px' }}>
          ...
          <FG label="Título *">
            <input style={inp} value={f.title} onChange={e => set('title', e.target.value)} placeholder="Ej: Implementar módulo de autenticación" />
          </FG>

          <FG label="Descripción">
            <textarea style={{ ...inp, height:70, resize:'vertical' }} value={f.description} onChange={e => set('description', e.target.value)} placeholder="Contexto, criterios de aceptación, notas…" />
          </FG>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:14 }}>
            ...
            <FG label="Etiqueta" mb={0}>
              <select style={inp} value={f.tag} onChange={e => set('tag', e.target.value)}>
                {TAGS.map(t => <option key={t}>{t}</option>)}
              </select>
            </FG>

            <FG label="Líder responsable" mb={0}>
              <select style={inp} value={f.leader_id} onChange={e => set('leader_id', e.target.value)}>
            ...
            <FG label="Fecha compromiso" mb={0}>
              <input type="date" style={inp} value={f.due_date} onChange={e => set('due_date', e.target.value)} />
            </FG>

          </div>

          <FG label="🗂️ Tipo de reunión">
            <input style={inp} value={f.meeting_type} onChange={e => set('meeting_type', e.target.value)} placeholder="Ej: Standup, Comité directivo, Revisión de sprint…" />
          </FG>

          <FG label="📅 Reunión de origen">
            <input style={inp} value={f.meeting_origin} onChange={e => set('meeting_origin', e.target.value)} placeholder="Ej: Reunión semanal 3 jun, Daily 5 jun…" />
          </FG>
